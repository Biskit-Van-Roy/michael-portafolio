...
export function Robotsaludo(props) {
  const group = useRef()
  const { scene, animations } = useGLTF('/models/robotsaludo.glb')
  ...
  useEffect(() => {
    ...
    names.forEach(name => {
      ...
      if (action) {
        action
          .reset()
          .setLoop(THREE.LoopRepeat, Infinity) // Set all animations to loop infinitely
          .fadeIn(0.1)
          .play()
      }
    })
    ...
    return () => {
      names.forEach(name => actions[name]?.stop()) // Clean up animations on unmount
    }
  }, [actions])

  useFrame((state) => {
    if (firstAnimationDone && camera.current) {
      const t = state.clock.getElapsedTime();
      const panningDuration = 2; // Duration of circular panning in seconds
      const radius = 10; // Radius of the circular path
      const targetPosition = new THREE.Vector3(0.2, -0.2, 0.2); // Final camera position
  
      if (t < panningDuration) {
        // Circular panning phase
        const angle = (t / panningDuration) * Math.PI * 2; // Complete one full circle
        camera.current.position.set(
          radius * Math.cos(angle), // x-coordinate
          0.5, // Fixed y-coordinate for simplicity (adjust if needed)
          radius * Math.sin(angle) // z-coordinate
        );
      } else {
        // Transition to final position
        camera.current.position.lerp(targetPosition, 0.05); // Smoothly interpolate to target
      }
  
      camera.current.lookAt(0, 0, 0); // Keep looking at the scene's center
    }
    camera.current = state.camera; // Update camera reference
  })
...
useGLTF.preload('/models/robotsaludo.glb')